...
\newcommand{\protoname}{Foo\xspace}
...
We assume that each participant is provisioned with an \emph{identity key} which can also be used
to compute digital signatures.  In the course of the protocol, each participant will also have a
\emph{leaf key} that can change over time.

\todokcg{Reusing the identity key as a signature key is a nasty hack that we did because Signal does
  it. I think a greenfield protocol should have separate keys: the identity key can be a signing
  key, and it can sign a DH key used in the protocol.}

\subsection{State}

\todokcg{This \S{} dives into a lot of the details without defining some of its concepts. We should
  probably define: agent, agent ID, epoch, conference, leaf key}

Logically, the state of a group $G$ comprises the following data:

\begin{itemize}
\item An \emph{epoch secret} $G.k$, the symmetric key derived by \protoname
\item An \emph{epoch number} $G.E$, incremented whenever $G.k$ is changed
\item A public \emph{group ID} $G.id$ that uniquely identifies the group within the scope of the messaging system
...
\item For each epoch, a set of shared secret values derived from the epoch secret:
	\begin{itemize}
	\item A \emph{message root key} $G.k_m$ used for deriving keys to protect messages
	\item An \emph{update secret}  $G.k_u$ used updating leaf keys
	\item An asymmetric \emph{add private key} $G.k_a$ used adding members to the group
	\end{itemize}
\end{itemize}

The Merkle trees are used to verify participants' membership in the group, while avoiding the need
to cache or transmit the full list of participants.  A participant must have the full participant
list in order to remove participants, and a delete message has $O(N)$ size, but all other messages
are of size $O(\log N)$.

\todokcg{Removals can probably be dealt with later, this is just initial state}
...
\todorlb{Note: Adds by the group actually do not need to be bound to an epoch; as long as they are applied in the same order, the message carries enough information for participants to rebase on a new epoch, rather than the sender having to do the rebase.}


\section{Signing and Roster Signing}

\todokcg{What is a roster?}

All messages in MLS are signed by the sender.  A \emph{roster signed} message also includes a proof of the signer's membership in the group.  A signed / roster signed message has the following components:
